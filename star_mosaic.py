...
class star_mosaic:
    ...
    def match_keypoints(self):
        ...
        def compute_signature_distance_partial(sig1, sig2, max_cost=np.pi / 4):
            ...
            cost_matrix = np.minimum(cost_matrix, 2 * np.pi - cost_matrix)  # Handle circular difference
            ...
            # Return average cost and match ratio
            match_ratio = valid_matches / max(len(sig1), len(sig2))
            return average_cost, match_ratio


        def find_matching_points_partial(plane1, plane2, cost_threshold=0.1, match_ratio_threshold=0.5):
            """
            Match points between two planes allowing for partial matches.
            ...
            # Match points based on their angular signatures
            widgets = [' [',
                    progressbar.Timer(),
                    '] ',
                    progressbar.Bar('*'),' (',
            ...
            bar = progressbar.ProgressBar(maxval=len(plane1_signatures), 
            ...
            for point1, signature1 in plane1_signatures.items():
                ...
                for point2, signature2 in plane2_signatures.items():
                    # Compute the distance and match ratio
                ...
                if best_match:
                    matches.append((point1, best_match, best_cost, best_ratio))
                bar.update((c:=c+1))
            return matches


        matches = find_matching_points_partial(img1_map, img2_map, cost_threshold=0.5, match_ratio_threshold=0.5)
